...
RedQueueDiscTestCase::RunRedTest (StringValue mode)
{
  ...
  // test 14: RED with default parameter settings, linear drop probability and fixed m_curMaxP and different minth and maxth setting
  queue = CreateObject<RedQueueDisc> ();
  minTh = 15 * modeSize;
  maxTh = 45 * modeSize;
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("Mode", mode), true,
  ...
  // test 15: MRED with linear drop probability
  queue = CreateObject<RedQueueDisc> ();
  minTh = 15 * modeSize;
  maxTh = 45 * modeSize;
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("Mode", mode), true,
